...
1. Make sure `fsc-utils` package referenced in the project is version `v1.33.2` or greater. Re-run `dbt deps` if revision was changed.
   ...
   \*\* NOTE: To circumvent a circular dependency issue when when installing FSC DBT packages into `livequery-models` the `livequery-models/deploy/core` models were moved to [livequery-base](https://github.com/FlipsideCrypto/livequery-base)
